...
\end{align}
El término central $P(C|E)$ es una medida descriptiva de las condiciones (usualmente datos) llamada \textit{verosimilitud}. En % REEEEFFFF
...
\begin{figure}[h]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{Chap3/Ej6BetaChain}
        \caption{Trazas}
        \label{fig:GibbsChain}
    \end{subfigure}
	\quad
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{Chap3/Ej6BetaHist}
        \caption{Histogramas}
        \label{fig:GibbsHist}
    \end{subfigure}
...
\end{scope}
	...
	\node at (-1.8,3.3) {Dados valores iniciales para $\pi(\beta)$ y $\pi(\wsn)$, se itera $k = 1,2,3,\ldots$};
...
\begin{scope}[
	...
	\path (w) edge [dashed, bend left](F);

\end{scope}

	\path (r) edge[dashed, bend left, ->] (w) node at (-4.8, -.7)
		{donde $\dashrightarrow$ se repite $d$ veces}(w);			
	\path (F) edge[bend left, dashed, ->] (r);
